function [Err, P] = tls(XData, YData, vizualization)
%
% function [Err, P] = fit_2D_data(XData, YData, vizualization)
%
% Orthogonal linear regression method (toal least-squares) in 2D for 
% model: y = a + bx   
%
% Input parameters:
...
%% Fixing bug
XData=XData+1e-3*randn(size(XData));
YData=YData+1e-3*randn(size(YData));

Y_std = std(YData);
X_std = std(XData);
if std(Y_std)>std(X_std)
    tmp=YData;
    YData=XData;
    XData=tmp;
end
